...
double PoroElasticity::getScaling(const Vec3& X, const TimeDomain& time) const
{
  if (sc == 0.0) {
                       ...
                       const TimeDomain& time, const Vec3& X) const
{
  ...
  Vector& eU = norms.vec[Vu];
  Vector& eP = norms.vec[Vp];
