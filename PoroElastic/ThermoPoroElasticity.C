                                     ...
                                     const TimeDomain& time, const Vec3& X) const
{
  ...
  const PoroMaterial* mat = this->getMaterial();
  ...
  if (!this->evalPermeabilityMatrix(Kpp, fe.grad(2),
                                    hydCond, scl1*scl1*fe.detJxW/(rhof*gacc)))
    return false;
  ...
  WeakOperators::Mass(CTT, fe, scl2*scl2*rhoc, 1, 1, 0, 2);
  ...
  WeakOperators::Laplacian(KTT, fe, scl2*scl2*lambda, 1, 1, false, 0, 2);
  WeakOperators::Advection(KTT, fe, vel, scl2*scl2*rhofcf, 1, 1, 0, 2);
